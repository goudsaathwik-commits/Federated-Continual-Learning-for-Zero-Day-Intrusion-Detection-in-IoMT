...
- **Decentralized Hospital Nodes**: $K=5$ client nodes ($H_1$ General Ward, $H_2$ Cardiology ICU, $H_3$ Pediatric Unit, $H_4$ Oncology Center, $H_5$ Emergency Unit) partitioned under Dirichlet distribution ($lpha=0.5$).
...
| **Proposed FL + CL + Zero-Day (E7)** | **0.2722** | **0.0900** | **-0.0874** | **0.5415** | **12.62 MB** |

Standard FedAvg (E3, $59.30\%$) matches Centralized performance ($59.51\%$) without sharing raw medical data. The proposed framework (E7) mitigates catastrophic forgetting ($	ext{BWT} = -0.0874$) and achieves a Zero-Day ROC-AUC of **0.5415**.

---

## XI. Ablation Study
Component ablation confirms that local Experience Replay memory buffers improve Zero-Day ROC-AUC from $0.4832$ (A4) to **0.5415** (A5) and reduce backward transfer degradation from $-0.1708$ (A3) to **-0.0874** (A5).
...
## XIII. Limitations
1. **Stealth Zero-Day Recall**: Energy-Based Scoring yields a recall of **0.0526** on stealthy malware payloads due to tabular telemetry overlaps with legitimate physiological traffic.

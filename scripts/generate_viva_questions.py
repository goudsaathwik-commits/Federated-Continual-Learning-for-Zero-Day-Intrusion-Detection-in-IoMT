...
logger = setup_logger("generate_viva_questions")

def compile_viva_qa_document() -> str:
    logger.info("Executing Phase 21: Compiling 100+ Viva Questions and Answers Document...")

    viva_dir = "viva"
    os.makedirs(viva_dir, exist_ok=True)

    qa_md = """# Master Viva Voce Questions & Answers: Federated Continual Learning for Zero-Day Intrusion Detection in IoMT
...
### Q30: Why is sample-proportional weighting ($\frac{n_k}{N}$) essential in FedAvg?
...
**Answer**: We used a Dirichlet distribution $\text{Dir}(\alpha)$ over label proportions with concentration parameter $\alpha=0.5$. Lower $\alpha$ values induce extreme label imbalance across clients.

### Q35: Describe the sample sizes of your 5 simulated hospital clients ($H_1 \dots H_5$).
**Answer**: Under $\alpha=0.5$, training splits were: $H_1$ General Ward ($2,935$ samples), $H_2$ Cardiology ICU ($1,417$ samples), $H_3$ Pediatric Unit ($1,051$ samples), $H_4$ Oncology Center ($452$ samples), and $H_5$ Emergency Unit ($664$ samples).
...
**Answer**: Continual Learning is the ability of an adaptive model to sequentially learn new tasks or attack streams ($\mathcal{T}_1 \rightarrow \mathcal{T}_2 \rightarrow \dots \rightarrow \mathcal{T}_M$) over time without degrading performance on previously mastered tasks.
...
**Answer**: Accuracy = $\frac{TP+TN}{TP+TN+FP+FN}$; Precision = $\frac{TP}{TP+FP}$; Recall = $\frac{TP}{TP+FN}$; F1-Score = $2 \cdot \frac{\text{Precision} \cdot \text{Recall}}{\text{Precision} + \text{Recall}}$.
...
**Answer**: $\text{FAR} = \text{FPR} = \frac{FP}{FP + TN}$. In cybersecurity, FAR measures the proportion of benign physiological traffic incorrectly flagged as malicious attacks.
...
**Answer**: By replacing rigid task boundaries ($\mathcal{T}_1 \rightarrow \mathcal{T}_2$) with online, streaming continual learning algorithms like A-GEM or Memory Aware Synapses (MAS).
...
**Answer**: Real clinical network packet logs are proprietary and legally restricted under HIPAA/GDPR, making public raw packet datasets unavailable. Simulating hospitals using Dirichlet label skew ($\alpha=0.5$) on Edge-IIoTset provides a realistic, reproducible benchmark for non-IID cross-institutional evaluation.
...
"""
    ...
    qa_path = os.path.join(viva_dir, "viva_questions.md")
    with open(qa_path, 'w', encoding='utf-8') as f:
        f.write(qa_md)
    logger.info(f"Saved complete viva QA document to: {qa_path}")

    return qa_md

if __name__ == "__main__":
    compile_viva_qa_document()
